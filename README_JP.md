...
[**最新バージョンをダウンロード**](https://github.com/gummidot/VRCLens-Addons/releases/latest)

> [!NOTE]
> **v1.xからアップグレードする場合:** v2.0でフォルダ構造が変更されました。`Assets/VRCLens_Custom` フォルダを丸ごと削除してから、新しいパッケージを再インポートしてください。その後、プレハブをアバターにドラッグし直してください。

## インストール

各アドオンは VRCFury プレハブで、アバターにドラッグ＆ドロップするだけでインストールできます。
好きなものだけを選んでインストールでき、プレハブを削除するだけで簡単に取り外せます。
すべてのアドオンはローカル専用、またはパラメータメモリを消費しません。
...
1. プレハブをアバターの `VRCLens` オブジェクトにドラッグしてください。
   - **注意:** VRCLens を再適用すると、プレハブが消えるため、再度ドラッグする必要があります。
...
> [!WARNING]
> これらは VRCLens の非公式アドオンであり、VRCLens のアップデートで動作しなくなる場合があります。古いバージョンや新しいバージョンでは動作しない可能性があります。
...
| プレハブ | 説明 |
|--------|-------------|
| [Smooth Rotate](#smooth-rotate) | カメラの動きをスムーズにする |
...
| [Player Visibility](#player-visibility) | VRCLens から他プレイヤーや自分を非表示にする |
...
| [Manual Focus (0.1m to 9m)](#manual-focus-01m-to-9m) | Manual Focus を0.1mまで下げられるようにする |
| [Manual Focus Assist](#manual-focus-assist) | フォーカスポイント付近のアバターのブラーを軽減 |
| [Max Blur Size](#max-blur-size) | パフォーマンス向上のため最大ブラーサイズを調整 |
...
| プレハブ | 説明 |
|--------|-------------|
| [Fix Avatar Drop](#fix-avatar-drop) | VRCLens 1.9.1以降で壊れた Avatar Drop を修正 |
| [VRCLens Optimizer](#vrclens-optimizer) | VRCLens のオプションコンポーネントを削除 |
| [Menu Extra](#menu-extra) | メニューの雑多な改善 |
...
デスクトップとVRの両方で動作し、OVR-SmoothTracking よりも大幅にスムージングできます。
...
- **Drop (Reset to Hand)** はカメラを手元に戻します。ドローンのDropボタンと同じで、便利のためにここにも配置しています。
...
VRCLens を改変し、デフォルトの最小値0.5mよりも近くにフォーカスできるようにします。Manual Focus の最大値も9mに制限され、軽いスムージングが追加され、オートフォーカスが無効になります。
...
**アバターAFを使用して Manual Focus ポイント付近のアバターのブラーを軽減**

Manual Focus 使用時にアバターをシャープに保つのを助けます。Avatar AF を有効にすると、フォーカス距離の前後2m以内のすべてのアバターのブラーが軽減されます。それ以外は通常通りにぼけます。
...
| Strength | 0% - 100% | 85% | ブラー軽減の度合い。0 = 効果なし、100 = 完全にシャープ。 |
...
| Peaking | On / Off | Off | 検出されたアバターにデバッグオーバーレイを表示: 緑 = ゾーン内（ブラー軽減）、黄 = 遷移ゾーン（部分的に軽減）、赤 = ゾーン外（フルブラー）。プレビューモードのみ表示。 |
...
**パフォーマンス向上のため最大ブラーサイズを調整**

DoF 使用時の最大ブラーサイズを調整できるローカル専用スライダーを追加します。パフォーマンスの改善（ブラーサイズを下げる = パフォーマンス向上）やぼかしの見た目を変更するのに使用できます。

#### 使い方

スライダーはメニューの `VRCLens/Custom/Max Blur Size` にあります。

0%ではスライダーの効果がなく、VRCLens インストール時のブラーサイズがそのまま使われます。0%以降は `Very Small` から `Very Large` までブラーサイズが増加します（各オプションについては VRCLens インストーラーを参照してください）。
...
物理フィルターのゴースト/モーションブラー効果を再現し、半透明でずれた像のコピーを生成します。Split モードは画面の半分に単方向のゴーストを作成します。Dual モードは両側からの二重ゴーストを作成し、中央にフレーミングした被写体はそのまま残ります。完全に回転可能です。
...
| Spread | 0% - 100% | 40% | マスキングがフォーカスポイントからどの程度広がるか（0.1〜50mシグマ）。低い値 = 狭いゾーン、高い値 = 木のような大きな被写体もカバー。 |
...
| 設定 | 範囲 | デフォルト | 説明 |
|---|---|---|---|
| Intensity | 0% - 100% | 0% | 手ブレの強さ。 |
| Speed | 0% - 100% | 30% | 手ブレアニメーションの速度。 |
| Distance | 0% - 100% | 0% | 手ブレがゴーストを動かす距離。 |
...
- **Depth Mode** -- シャープネスが画面上の位置ではなく、カメラからの距離に基づきます。タワーのような背の高いオブジェクトは根元から頂上までシャープに保たれます。VRCLens の Manual Focus またはオートフォーカスに連動します。背の高い建物や木のあるシーンに最適。
...
**VRCLens 1.9.1以降で壊れた Avatar Drop 機能を修正**

Avatar Drop は VRCLens 1.9.1以降（VRCLens 1.9.2 時点）でバグがあります。このプレハブで自動的に修正されます。
...
`VRCLensOptimizer` プレハブをアバターの `VRCLens` オブジェクトにドラッグ＆ドロップしてください。削除したいコンポーネントにチェックを入れてください。コンポーネントはアップロード時に削除されるため、実際のマテリアル/ポリゴン数はゲーム内のアバターステータスで確認してください。
...
### VRCLens を VRCFury プレハブとして使う

VRCLens はアバターのFXコントローラー、メニュー、パラメータを直接変更するため、VRCLens あり/なしの異なるアバターバージョンを共有したり管理するのが難しくなります。

VRCLens を VRCFury プレハブに変換すれば、一度セットアップするだけで、さまざまなアバターバージョンにドラッグ＆ドロップで適用でき、簡単に削除できます。

これは手動のプロセスです。ガイドはこちら: https://gummidot.notion.site/VRCLens-as-a-VRCFury-Prefab-15623187a377802fbc17d0357e56f8bc