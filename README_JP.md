...
各アドオンは VRCFury Prefab で、アバターにドラッグ＆ドロップするだけでインストールできます。
好きなものだけを選んでインストールでき、Prefab を削除するだけで簡単に取り外せます。
すべてのアドオンはローカル専用で、同期パラメータメモリを消費しません。
...
1. Prefab をアバターの `VRCLens` オブジェクトにドラッグしてください。
